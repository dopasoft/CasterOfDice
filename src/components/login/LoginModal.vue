<template>
  <a-modal v-model="visible" :forceRender="true" :bodyStyle="bodyStyle" :width="1090" :footer="null" :maskClosable='false' dialogClass="wrapClass">
  ...
  watch: {
    '$store.state.loginMode': {
      handler: function (to, from) {
        if (to === DicEnum.loginMode.SIGNUP) {
          this.isSignup = true
          this.isLogin = false
          this.isForgot = false
          this.visible = true
        } else if (to === DicEnum.loginMode.LOGIN) {
          this.isSignup = false
          this.isLogin = true
          this.isForgot = false
          this.visible = true
        } else if (to === DicEnum.loginMode.FORGOT) {
          this.isSignup = false
          this.isLogin = false
          this.isForgot = true
          this.visible = true
        } else if (to === DicEnum.loginMode.CLOSE) {
          this.visible = false
        }
        this.$store.commit('changLogState', DicEnum.loginMode.UNSET)
      },
      immediate: true
    }
  },
